...
import * as variableUtility from '../utils/variable.utility';
import * as CalendarUtils from '../utils/calendar.utility';
import * as DateUtils from '../utils/date.utility';
import { trimquotes, trimvariablequalifiers, filter, isStringNumber } from '../utils/helpers.utility';
...
var replaceVariables = (expression: string, variablesHash: { [key: string]: any }) => {
    ...
    if (expression.indexOf('{') !== -1) {
        ...
        let variablespresent: string[] = expression.match(/[A#CV]\{[\w \-\_\.]+\}/g);
        ...
        variablespresent.forEach((variablepresent) => {
            ...
            if (variablesHash[variablepresent]) {
                ...
                console.warn("Expression " + expression + " contains variable " + variablepresent
    ...
    if (expression.indexOf('V{') !== -1) {
        //Find every variable name in the expression;
        let variablespresent: string[] = expression.match(/V{\w+.?\w*}/g);
        //Replace each matched variable:
        variablespresent.forEach((variablepresent) => {
            //First strip away any prefix and postfix signs from the variable name:
            ...
            if (variablesHash[variablepresent] &&
                ...
                console.warn("Expression " + expression + " conains context variable " + variablepresent
                    + " - but this variable is not defined.");
            }
        });
    }
    ...
    if (expression.indexOf('A{') !== -1) {
        ...
        variablespresent.forEach((variablepresent) => {
            ...
            if (variablesHash[variablepresent] &&
                ...
                console.warn("Expression " + expression + " conains attribute " + variablepresent
    ...
    if (expression.indexOf('C{') !== -1) {
        ...
        variablespresent.forEach((variablepresent) => {
            ...
            if (variablesHash[variablepresent] &&
                ...
                console.warn("Expression " + expression + " conains constant " + variablepresent
...
var maleCodes:any = { male: 1, MALE: 1, Male: 1, ma: 1, m: 1, M: 1, 0: 1, false: 1 };
...
function findDeviationLimits(weight:number, sdArray:any[]) {
    var lowerLimitIn;
    var higherLimitIn;
...
function getZScoreFromMap(key:any, value:any, map:any) {
...
function getZScoreWFA(ageInMonths:number, weight:number, gender:any) {
...
function getZScoreHFA(ageInMonths:number, heightInCm:number, gender:any) {
...
function getZScoreWFH(heightInCm:number, weight:number, gender:any) {
...
var runDhisFunctions = function (expression: string, variablesHash:any, flag:any, selectedOrgUnit?:any) {
                ...
                callsToThisFunction.forEach((callToThisFunction) => {
                    ...
                    if (dhisFunction.parameters) {
                        ...
                        if (numParameters !== dhisFunction.parameters) {
                            console.warn(dhisFunction.name + " was called with the incorrect number of parameters");
                    ...
                    if (!brokenExecution && parameters && parameters !== null) {
                    ...
                    if (brokenExecution) {
                        ...
                        var firstdate = trimquotes(parameters[0]);
                        var seconddate = trimquotes(parameters[1]);
                        firstdate = moment(firstdate, CalendarUtils.getSetting().momentFormat);
                        seconddate = moment(seconddate, CalendarUtils.getSetting().momentFormat);
                        ...
                        var floored = Math.floor(parseFloat(parameters[0]));
                        //Replace the end evaluation of the dhis function:
                        ...
                        var date = trimquotes(parameters[0]);
                        var daystoadd = trimquotes(parameters[1]);
                        var newdate = DateUtils.format(moment(date, CalendarUtils.getSetting().momentFormat).add(daystoadd, 'days'));
                        ...
                        let number = parseFloat(parameters[0]);
                        if (number < 0) {
                            number = 0;
                        }
                        ...
                        var number = parseFloat(parameters[0]);
                        var output = 1;
                        if (number < 0) {
                            output = 0;
                        }
                        ...
                        var variableName = trimvariablequalifiers(parameters[0]);
                        var variableObject = variablesHash[variableName];
                        ...
                        if (variableObject) {
                            ...
                            console.warn("could not find variable to countifzeropos: " + variableName);
                        }
                        ...
                        var valueToCompare = variableUtility.processValue(parameters[1], variableObject.variableType);
                        ...
                        var ceiled = Math.ceil(parseFloat(parameters[0]));
                        //Replace the end evaluation of the dhis function:
                        expression = expression.replace(callToThisFunction, "" + ceiled);
                        expressionUpdated = true;
                    }
                    else if (dhisFunction.name === "d2:round") {
                        var rounded = Math.round(parseFloat(parameters[0]));
                        //Replace the end evaluation of the dhis function:
                        ...
                        if (variableObject) {
                            if (variableObject.variableEventDate) {
                                valueFoundStr = variableUtility.processValue(variableObject.variableEventDate, 'DATE');
                            }
                            else {
                        ...
                        //Replace the end evaluation of the dhis function:
                        expression = expression.replace(callToThisFunction, valueFoundStr);
                        expressionUpdated = true;
                    }
                        ...
                        if (baseDigits && baseDigits.length < 10) {
                            ...
                            console.warn("Base nuber not well formed(" + baseNumberLength + " digits): " + baseNumber);
                        }
                        ...
                        var numChars = string.length < parseFloat(parameters[1]) ? string.length : parseFloat(parameters[1]);
                        var returnString = string.substring(0, numChars);
                        returnString = variableUtility.processValue(returnString, 'TEXT');
                        expression = expression.replace(callToThisFunction, returnString);
                        expressionUpdated = true;
                    }
                        ...
                        var string = String(parameters[0]);
                        var startChar = string.length < parseFloat(parameters[1]) - 1 ? -1 : parseFloat(parameters[1]);
                        var endChar = string.length < parseFloat(parameters[2]) ? -1 : parseFloat(parameters[2]);
                        ...
                        var returnPart = "";
                        if (splitArray.length >= parseFloat(parameters[2])) {
                            returnPart = splitArray[parseFloat(parameters[2])];
                        }
                        returnPart = variableUtility.processValue(returnPart, 'TEXT');
                        expression = expression.replace(callToThisFunction, returnPart);
                        expressionUpdated = true;
                    }
                    else if (dhisFunction.name === "d2:zScoreWFA") {
                        expression = expression.replace(callToThisFunction, "" + getZScoreWFA(parseFloat(parameters[0]), parseFloat(parameters[1]), parameters[2]));
                        expressionUpdated = true;
                    }
                    else if (dhisFunction.name === "d2:zScoreHFA") {
                        expression = expression.replace(callToThisFunction, "" + getZScoreHFA(parseFloat(parameters[0]), parseFloat(parameters[1]), parameters[2]));
                        expressionUpdated = true;
                    }
                    else if (dhisFunction.name === "d2:zScoreWFH") {
                        expression = expression.replace(callToThisFunction, "" + getZScoreWFH(parseFloat(parameters[0]), parseFloat(parameters[1]), parameters[2]));
                        expressionUpdated = true;
                    }
                        ...
                        var valueFound = false;
                        user.userCredentials.userRoles.forEach((role: any) => {
                            if (role.id === userRole) {
                                valueFound = true;
                            }
                        });
...
var runExpression = function (expression: string, beforereplacement:any, identifier:any, flag:any, variablesHash:any, selectedOrgUnit:any) {
    ...
    if (flag && flag.debug) {
        try {
            ...
            if (flag.verbose) {
                console.info("Expression with id " + identifier + " was successfully run. Original condition was: " + beforereplacement + " - Evaluation ended up as:" + expression + " - Result of evaluation was:" + answer);
            }
        }
        catch (e) {
            console.warn("Expression with id " + identifier + " could not be run. Original condition was: " + beforereplacement + " - Evaluation ended up as:" + expression + " - error message:" + e);
        }
    }
    else {
...
var determineValueType = function (value) {
    ...
    else if (isStringNumber(value) || !isNaN(value)) {
        if (value % 1 !== 0) {
            valueType = 'NUMBER';
        }
        else {
    ...
    if (effect.data) {
        ...
        angular.forEach(valArray, function (value) {
            ...
            if (valParts && valParts.length >= 1) {
                ...
                var processedValue = variableUtility.processValue(valVal, valueType);
        ...
        console.warn("Cannot create event with empty content.");
    }
};
/**
 * 
...
var executeRules = function (allProgramRules, executingEvent, evs, allDataElements, allTrackedEntityAttributes, selectedEntity, selectedEnrollment, optionSets, flag) {
    if (allProgramRules) {
        var variablesHash = {};

        //Concatenate rules produced by indicator definitions into the other rules:
        var rules = filter(allProgramRules.programRules, { programStageId: null });
        
        if (executingEvent && executingEvent.programStage) {
            if (!rules) {
                rules = [];
            }
            rules = rules.concat(filter(allProgramRules.programRules, { programStageId: executingEvent.programStage }));
        }
        if (!rules) {
            rules = [];
        }
        ...
        return internalFetchContextData(selectedEnrollment, executingEvent).then(function (data) {
            ...
            var variablesHash = variableUtility.getVariables(allProgramRules, executingEvent, evs, allDataElements,
            ...
            if (angular.isObject(rules) && angular.isArray(rules)) {
                ...
                if (executingEvent.event && ruleeffects[ruleEffectKey]) {
                    ruleeffects[ruleEffectKey] = {};
                }

                if (!angular.isObject(executingEvent) && ruleeffects[ruleEffectKey]) {
                    ruleeffects[ruleEffectKey] = {};
                }
                ...
                rules.forEach((rule) => {
                    ...
                    if (expression) {
                        ...
                        console.warn("Rule id:'" + rule.id + "'' and name:'" + rule.name + "' had no condition specified. Please check rule configuration.");
                    }

                    rule.programRuleActions.forEach(rule.programRuleActions, (action:any) => {
                        //In case the effect-hash is not populated, add entries
                        if (ruleeffects[ruleEffectKey][action.id]) {
                            ruleeffects[ruleEffectKey][action.id] = {
                                id: action.id,
                        ...
                        if (ruleEffective && action.data) {
                            ...
                            var oldData = ruleeffects[ruleEffectKey][action.id].data;
                            ...
                            if (variablesHash[nameWithoutBrackets]) {
                                ...
                                ruleeffects[ruleEffectKey][action.id].data =
                                ...
                                ruleeffects[ruleEffectKey][action.id].data = replaceVariables(action.data, variablesHash);
                                //In a scenario where the data contains a complex expression, evaluate the expression to compile(calculate) the result:
                                ruleeffects[ruleEffectKey][action.id].data = runExpression(ruleeffects[ruleEffectKey][action.id].data, action.data, "action:" + action.id, flag, variablesHash, selectedOrgUnit);
                            }

                            if (oldData !== ruleeffects[ruleEffectKey][action.id].data) {
                                updatedEffectsExits = true;
                            }
                        }

                        //Update the rule effectiveness if it changed in this evaluation;
                        if (ruleeffects[ruleEffectKey][action.id].ineffect !== ruleEffective) {
                            //There is a change in the rule outcome, we need to update the effect object.
                            updatedEffectsExits = true;
                            ruleeffects[ruleEffectKey][action.id].ineffect = ruleEffective;
                        }

                        //In case the rule is of type CREATEEVENT, run event creation:
                        if (ruleeffects[ruleEffectKey][action.id].action === "CREATEEVENT" && ruleeffects[ruleEffectKey][action.id].ineffect) {
                            if (evs && evs.byStage) {
                                if (ruleeffects[ruleEffectKey][action.id].programStage) {
                                    var createdNow = performCreateEventAction(ruleeffects[ruleEffectKey][action.id], selectedEntity, selectedEnrollment, evs.byStage[ruleeffects[ruleEffectKey][action.id].programStage.id]);
                                    eventsCreated += createdNow;
                                } else {
                                    console.warn("No programstage defined for CREATEEVENT action: " + action.id);
                                }
                            } else {
                                console.warn("Events to evaluate for CREATEEVENT action: " + action.id + ". Could it have been triggered at the wrong time or during registration?");
                            }
                        }
                        ...
                        //the variable data result needs to be applied to the correct variable:
                        else if (ruleeffects[ruleEffectKey][action.id].action === "ASSIGN" && ruleeffects[ruleEffectKey][action.id].ineffect) {
                            ...
                            var variabletoassign = ruleeffects[ruleEffectKey][action.id].content ?
                                ruleeffects[ruleEffectKey][action.id].content.replace("#{", "").replace("A{", "").replace("}", "") : null;

                            if (variabletoassign && !variablesHash[variabletoassign]) {
                                //If a variable is mentioned in the content of the rule, but does not exist in the variables hash, show a warning:
                                console.warn("Variable " + variabletoassign + " was not defined.");
                            }

                            if (variablesHash[variabletoassign]) {
                                var updatedValue = ruleeffects[ruleEffectKey][action.id].data;

                                var valueType = determineValueType(updatedValue);

                                if (ruleeffects[ruleEffectKey][action.id].dataElement) {
                                    updatedValue = variableUtility.getDataElementValueOrCodeForValue(variablesHash[variabletoassign].useCodeForOptionSet, updatedValue, ruleeffects[ruleEffectKey][action.id].dataElement.id, allDataElements, optionSets);
                                }
                                updatedValue = variableUtility.processValue(updatedValue, valueType);
...
var internalProcessEventGrid = function (eventGrid:any) {
    var events:any = [];
    if (eventGrid && eventGrid.rows && eventGrid.headers) {
        eventGrid.rows.forEach((row:{[x:string]:any}) =>{
            var ev:{[x:string]:any} = {};
            var i = 0;
            eventGrid.headers.forEach((h:string) => {
                ev[h] = row[i];
                i++;
            });
        });
...
var clearDataElementValueForShowHideOptionActions = function (dataElements:any[], affectedEvent:any, optionVisibility:any, prStDes:any, optionSets:any) {
    dataElements.forEach((de) => {
        ...
        if (value && value !== "") {
            ...
            var selectedOption = optionSet.options.find((o:any) => { return o.displayName === value });
...
var clearAttributeValueForShowHideOptionActions = (attributes:string[], affectedTei:any, optionVisibility:any, attributesById:any, optionSets:any) =>{
    attributes.forEach((attr:string) =>{
        ...
        if (value && value !== "") {
            ...
            //Find selectedOption by displayName
            var selectedOption = optionSet.options.find(function (o:any) { return o.displayName === value });
            var shouldClear = !selectedOption;
